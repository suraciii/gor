...
内存假网络 + `testing/synctest` 假时钟 + porcupine 做线性一致性检查。
...
**验收**：能用一个固定种子复现一次注入的网络分区，并且重跑得到逐字节相同的事件序列。
...
**验收**：进程崩溃后到期任务仍会触发；同一任务不会重复触发。

### 6. 多节点

一致性哈希环 + 共享表 membership + 死亡投票。
...
**验收**：DST 场景覆盖节点加入、节点崩溃、网络分区、分区恢复；每个场景下不变量断言不破。
...
- 文档英文化。
- 一个真实的示例应用（不是 counter 那种 hello world）。
- 性能基线数字，且要说清测量条件。
