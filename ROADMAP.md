...
- Performance baseline numbers and the cross-node forwarding baseline. **Blocked on the candidate commit:** the numbers in [benchmarks.md](benchmarks.md) were measured before step 6c's probe-config validation landed, and `make bench` now fails because `BenchmarkForwardingRoundTrip` does not pass the six probe options that `cluster.New` requires (zero values return `cluster.ErrInvalidConfig`). The forwarding baseline cannot be re-measured until the benchmark is fixed; this must happen before `v0.1.0`. What is measured, what is not, and how conditions are written: [design/benchmarks.md](design/benchmarks.md).
