...
**6b 模拟器差距：heal 的心跳夹具。** 当前分区模型给每个节点一份私有的成员表副本；分区期间的 heartbeat 只写入各自副本。`heal` 丢弃副本时，分区期间写入的 heartbeat 也随之丢失，共享成员表看起来就像整个分区期间没有心跳。若直接让 6a 基于 `iam_alive_at` 的陈旧判死继续运行，它会把只是被隔离且仍健康的节点杀掉。模拟器因此在 `heal` 时向共享表补写当前 fake time 的 active heartbeat，抵消这个模型失真。这是测试夹具，不是被测运行时的行为；6c 移除 6a 的粗判死后应删除这项补偿，并让测试在没有它的情况下通过。
