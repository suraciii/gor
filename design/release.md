...
Every "required before release" item except the last one is done: English documentation, public API doc comments, the error and cancellation contract, the root runtime shutdown contract, deactivation reasons and the background error sink, the example application, and observability. The performance baseline numbers are recorded in [benchmarks.md](../benchmarks.md), but the cross-node forwarding baseline is not currently reproducible: `make bench` fails on the candidate commit because `BenchmarkForwardingRoundTrip` has not passed the six probe options that `cluster.New` has required since step 6c, so that item is not cleanly done until the benchmark is fixed. What remains is the "Versioning and release" item itself: the release process this document describes, including the install-and-upgrade check in release-sequence step 3.
...
The release-prep audit also found an unresolved documentation inconsistency: the cluster startup snippet in [docs/programming-model.md](../docs/programming-model.md) omits the six probe options that a clustered `gor.New` requires, so a verbatim copy fails with `cluster config is invalid`, while [design/cluster.md](cluster.md) still lists default values for those options that the implementation does not apply. Resolving this (implement the defaults, or document the required options and correct the design table) is a maintainer decision; until then the documented cluster startup flow is not trustworthy.
