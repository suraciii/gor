...
The install half of release-sequence step 3 was run on the pseudo-version `v0.0.0-20260806023618-833ef18b818d` in a clean user module outside the repository: following the documented flow with default flags — `go get` of the module, `go get -tool` of the generator, `go tool gorgen -pkg ./domain` without `-out`, import of the generated `<entity-pkg>/gorgen`, a minimal entity flow, close, reopen, and state survives — passes end to end with no extra steps. The two gaps previously listed here (a missing go.sum entry for `golang.org/x/tools` and the unimportable `internal/gorgen` default) are gone.
