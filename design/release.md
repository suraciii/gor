...
The upgrade half of release-sequence step 3 was backfilled for the `v0.0.1` → `v0.0.2` cut, which the codegen import-alias change in v0.0.2 triggered. In a clean user module, a `v0.0.1` project wrote entity state and a pending scheduled task, then swapped the library and generator to `v0.0.2` and reopened the same store: state read back unchanged, the pending task delivered on the new version, and regenerated call artifacts were byte-identical to the `v0.0.1` output. The only breaking change in v0.0.2 is the scheduled-task typed handle — `Schedule`/`NewSchedule` became generic and `Set` takes `gor.Handle(T.Method)` instead of a method-name string. Each `Set` call site and the `Schedule` declaration must change; the line count depends on how many tasks a user scheduled — three lines in the minimal check project. That is allowed in the 0.0.x band, but it is the one place users must touch; no other incompatibility surfaced. Release-sequence step 6 (install the exact tagged version in a clean project after tagging) becomes meaningful once there is an announced release; for 0.0.x the pre-tag install check in step 3 already covers installability.
