...
`Set()` 拿到 `ErrConflict` 时，做两件事：

1. **内存里的值不变。** 写没成功，格子就不能装作写成功了，否则内存和存储从此对不上，而且用户读到的是一个存储里根本不存在的值。
2. **停用这个激活。** 它手里的 ETag 已经过期了，后面每一次写都会撞同一堵墙。下次调用重新从 store 读，拿到新的 ETag 再继续。
