...
Transport teardown does not yet meet the invariant above. In a graceful stop, `closeTransport` runs after `<-engine.Done()` and `waitDrained()`, and `waitDrained` gates on the root inflight — but the inflight releases when the serving handler returns. That is method completion, not reply delivery: the reply still has to cross the transport back to the caller, and `closeTransport` (which calls an abrupt `Transport.Close`) can run in that window and truncate it. The initiating side is correct: a forwarded call holds the initiator's root inflight until `transport.Send` returns, so the initiator does not close its own transport early. The gap is on the serving side, and it is not closed by holding the owner's inflight longer — the runtime cannot learn when the peer received the reply. It is closed at the transport layer, by making the graceful close flush in-flight replies before tearing down; see [transport.md](transport.md). The two-node fake-network test that was meant to verify this invariant is flaky rather than deterministic; the deterministic check is specified in [transport.md](transport.md) and is not yet in place.
