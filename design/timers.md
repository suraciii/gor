...
**但不重试不等于不吭声。** 定时投递没有调用方在等，方法返回的 error 无处可去，轮询器现在把它丢掉。丢掉的后果是一整类故障对用户完全不可见：闹钟响了、方法失败了、状态没改成，而系统看起来一切正常。

所以配置上要有一个错误出口：

```go
OnTimerError func(id Identity, method string, err error)
```

一个出口，不是每个定时任务一个。它不做重试、不做退避、不做告警策略——那些是用户的事，运行时只负责把「这件事失败了」送到用户手上。

不配就是丢掉，跟现在一样。**这是唯一一处运行时替用户吞掉错误的地方，所以它必须在文档里写明，而不是藏在实现里。**
