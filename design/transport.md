...
Under the `sim` build tag, `simulationTransport` can drop individual messages by a seed-drawn decision, drop a whole node pair across a partition, and delay messages for a seeded number of fake-clock ticks. Reorder is not a distinct fault under this transport model (see [simulation.md](simulation.md)).
