...
**判死是先写后信。** 本地算出「这一行的 `iam_alive_at` 陈旧了」不能直接进视图，要等那次 CAS 成功。写没写成就当对方还活着。

反过来会出一个表上完全看不出异常的坏局面：两个节点各自认定对方陈旧，两次 CAS 都没落地，于是谁都没被判死、谁都不会自杀，但两份视图里都只剩自己——每个 key 都有两个属主，而表上两行都好端端地写着 `active`。
